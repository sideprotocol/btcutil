...
func IsBitCoinAddr(bech string) string {
	if strings.HasPrefix(bech, BITCOIN_HRP+"1q") && len(bech) == 42 {
		return "segwit"
	} else if strings.HasPrefix(bech, BITCOIN_HRP+"1p") && len(bech) == 62 {
		return "taproot"
	}
	return "cosmos"
}
